...
vec <- c(rnorm(100), runif(100), rnorm(100, 1)) # a vector with three "groups" - normals, uniforms and normals with mean 1
levels <- gl(3, 100)                            # factor that will categorize the groups in v
tapply(vec, fact, mean, simplify=TRUE)   # returns the mean for each group
tapply(vec, fact, range, simplify=FALSE) # returns a list where each element is the range of one of the groups
